    ...
    @if (Auth::user()->tipo == 'Administrador' || Auth::user()->tipo == 'Gerente')
        <div class="card card-primary card-outline">
            <div class="card-header">
                <h3 class="card-title m-0"> Editar Formação => {{ $formacao->nome }}</h3>
            ...
            <div class="card-body">
                ...
                {!! Form::model($formacao, ['method' => 'PATCH', 'route' => ['formacao.update', $formacao->idformacao]]) !!}
                {{ Form::token() }}
                <div class="form-group">
                    <label for="nome">Formação</label>
                    <input type="text" name="nome" value="{{ $formacao->nome }}" class="form-control" required>
                </div>
                <div class="form-group">
                    <label for="descricao">Descrição</label>
                    <input type="text" name="descricao" value="{{ $formacao->descricao }}" class="form-control">
                </div>
                <div class="form-group">
